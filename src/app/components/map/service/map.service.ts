...
export class MapService {
  ...
  constructor(
    ...
    this.markerinmapList = this.getMarkerinmap();
    this.uploadInfo();
  }
  ...
  insertMarkerinmap(coordinates: number[], category: Category) {
    this.markerinmapList.push({
      id: Guid.create(),
      latitude: coordinates[0],
      longuitude: coordinates[1],
      category: category,
      date: new Date()
    });
  }
  ...
  insertMarker(coordinates: number[], category: Category) {
    this.markers.push(new GeoJson(Guid.create(), new Date(), coordinates, category));
  }
